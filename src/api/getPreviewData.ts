export async function getPreviewData(type: string) {
  ...
  const res = await fetch(
    `https://admin.planetearthnow.org/api/${type}?publicationState=preview&populate=deep`,
    {
/*       headers: {
...
export async function getPreviewDataAction(type: string, entryId: string) {
  ...
  // TODO : if type, /${entryId}
  const res = await fetch(
    `https://admin.planetearthnow.org/api/${type}/${entryId}?publicationState=preview&populate=deep`,
    {
/*       headers: {
        Authorization: `Bearer ${secret}`,
      }, */
